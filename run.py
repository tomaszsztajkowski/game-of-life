...
def main():
    def next_frame():
        ...
        for cell in conway.currently_alive:
            x, y = cell
            if x - offset_x > SCREEN_SIZE_X // box_offset + 1 or x - offset_x < -1:
                continue
            if y - offset_y > SCREEN_SIZE_Y // box_offset + 1 or y - offset_y < -1:
                continue

            rectangle = (
                (x - offset_x) * box_offset + g,
                (y - offset_y) * box_offset + g,
                box_offset - g,
                box_offset - g,
            )
        ...
        pygame.draw.rect(
            window, BLACK, (SCREEN_SIZE_X, 0, SIDE_PANNEL_WIDTH + 1, SCREEN_SIZE_Y)
        )
    ...
    def load_image(name):
        ...
        for y in range(cells.shape[0]):
            for x in range(cells.shape[1]):
                if cells[y][x] == 1:
                    conway.currently_alive.add((x, y))
    ...
    while True:
        if generating:
            time_point += clock.tick(FRAMERATE_LIMIT)
        else:
            pass
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                coor = (
                    mouse_position[1] // box_offset,
                    mouse_position[0] // box_offset,
                )
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    bitmap_names = listdir("patterns")
                    patterns = [
                        font.render(" " + p[:-4].upper(), True, WHITE)
                        for p in bitmap_names
                    ]
                ...
                if event.button == 1 and mouse_position[0] > SCREEN_SIZE_X:
                    try:
                        offset_x, offset_y = load_image(
                            bitmap_names[mouse_position[1] // FONT_SIZE]
                        )
                        generating = False
                        time_point = 0
                        box_offset = BOX_SIZE
                    ...
                    x, y = (
                        pygame.mouse.get_pos()[0] / box_offset,
                        pygame.mouse.get_pos()[1] / box_offset,
                    )
                    if event.button == 4:
                        box_offset += 1
                    elif event.button == 5:
                    ...
                    offset_x = round(
                        offset_x + x - pygame.mouse.get_pos()[0] / box_offset
                    )
                    offset_y = round(
                        offset_y + y - pygame.mouse.get_pos()[1] / box_offset
                    )
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            if coor[1] < -offset_x + 1:
                ...
                conway.currently_alive = {
                    (cell[0] + dist, cell[1]) for cell in conway.currently_alive
                }
                offset_x += dist
            elif coor[0] < -offset_y + 1:
                dist = -(coor[0] + offset_y) + 1
                conway.currently_alive = {
                    (cell[0], cell[1] + dist) for cell in conway.currently_alive
                }
                offset_y += dist
            ...
            if not prev_mouse_position:
                ...
                mouse_y = mouse_x = 0
            elif prev_mouse_position == mouse_position:
                pass
            else:
